...
class EmailSender
{
    ...
    void Send(string? fromEmail, string? fromName, string? subjectOverride, string emailTemplateId, string? replyToEmail)
    {
        ...
        {
            From = ((Func<string, EmailSender, string, EmailAddress>)(static (fromEmail, emailSender, fromName) => new EmailAddress(
                fromEmail ?? emailSender._emailSettings.Defaults.FromEmail,
                fromName ?? emailSender._emailSettings.Defaults.FromName
