...
Strongly prefer to use the refactoring tool rather than editing files. It is available as MCP tools (one per command below); use those instead of shelling out. If MCP tools aren't available, fall back to `dotnet run --project RoslynRefactor -- {COMMAND} {OPTIONS}`.
